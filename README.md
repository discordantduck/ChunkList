...
1. ***What is the advantage of the ChunkList approach as opposed to a standard 
  the link list implementation?*** (1 point)
   > searching for a node at the given index is faster because rather than going
   through every node, you can skip whole chunks of nodes.
...
2. ***What would be the implications of increasing the size of ARRAY_SIZE to a 
   ...
   > it means searching for a node at the given index is faster, because you can
   skip larger chunks of nodes, but it'll probably be more wasteful in terms of
   memory, because the memory is freed when 1000 slots are empty rather than 8.
...
4. ***Compare placing a new element into the *FIRST* available empty space 
   ...
   > for the first option, you have to search the list for an empty slot, which means
   the worst case will always be worse than just skipping straight to the tail node and
   appending the value there. The benefit is that you are more efficient with memory usage.
